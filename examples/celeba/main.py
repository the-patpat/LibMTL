...
def main(params):
    ...
    trainloader = torch.utils.data.DataLoader(
        CelebA(root=params.dataset_path, split='train', 
                             transform=t),
    ...
    valloader = torch.utils.data.DataLoader(
        CelebA(root=params.dataset_path, split='valid',
                           transform=t),
        batch_size=params.train_bs, 
        shuffle=True, 
        num_workers=0, 
    ...
    testloader = torch.utils.data.DataLoader(
        CelebA(root=params.dataset_path, split='test',
                           transform=t),
        batch_size=params.train_bs, 
        shuffle=True, 
        num_workers=0, 
    ...
    # define tasks 
    task_dict = {
        attr_name : {'metrics': ['classAcc'], 
               'metrics_fn' : AccMetric(), 
               'loss_fn' : CELoss(), 
               'weight' : [1]
        } for attr_name in CelebA(root=params.dataset_path, split='train').attr_names if len(attr_name) > 0
        }
    ...
    encoder_class = lambda : resnet.resnet18(pretrained=params.pretrained).to(f'cuda:{params.gpu_id}')
    decoders = {task : nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                    nn.Flatten(1),
                                    nn.Linear(512, 50),
                        celeba_head.CelebAHead(params.dropout, n_classes=2)).to(f'cuda:{params.gpu_id}')
            for task in task_dict.keys()}
    ...
    if params.mode == 'train':
        CelebAModel.train(trainloader, None, params.epochs,
                         val_dataloaders=valloader)
    elif params.mode == 'test':
        CelebAModel.test(testloader)
    else:
        raise ValueError
